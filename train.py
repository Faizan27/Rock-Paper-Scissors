...
# Train the classifier using grayscale features
...
def train(nbImg=0, cvScore=True):
    ...
    sssplit = StratifiedShuffleSplit(n_splits=1, test_size=.15, random_state=random_states)
    ...
    cv = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=random_states)
    ...
    if cvScore:
        ...
        for score, std, params in zip(cvResults['mean_test_score'],
                cvResults['std_test_score'], cvResults['params']):
    ...
    print('+{}s: Writing classifier to {}'.format(dt(), pickleFileName))
